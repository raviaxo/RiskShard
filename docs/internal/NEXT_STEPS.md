...
**Updated 2026-08-07.** ADR-0007 (construct coherence) shipped on branch
`shard/construct-coherence` — declaration and measurement only, unmerged, no PR yet.
Headline: **4 coherent · 18 mixed of 22 parameter families; every shard carries at least one
mixed family and no impact family is coherent.** The next objectives, in order:
...
2. **Wire coherence into the public surfaces** — explorer per-row status, evidence report,
   cover facts, readiness dashboard. Currently CLI-only, so the honest count is not yet
   published anywhere a reader sees. Mirrors what ADR-0003 part 2 did for population.
