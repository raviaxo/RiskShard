...
class BenchmarkProgramTests(unittest.TestCase):
    def test_benchmark_program_tracks_thirty_targets_without_overclaiming(self):
        ...
        self.assertEqual(report["module_seeded_count"], 10)
        self.assertEqual(report["benchmark_ready_count"], 3)
        self.assertEqual(report["status_counts"]["missing_module"], 20)
        self.assertEqual(report["status_counts"]["needs_evidence"], 7)
        self.assertIn("Benchmark-Grade 30 Shard Program", output)
        self.assertIn("benchmark-ready: 3", output)
    ...
    def test_seeded_cohort_report_ranks_large_upgrade_work(self):
        ...
        self.assertEqual(cohort["target_count"], 10)
        self.assertEqual(cohort["benchmark_ready_count"], 3)
        self.assertEqual(cohort["status_counts"]["benchmark_ready"], 3)
        self.assertEqual(cohort["status_counts"]["needs_evidence"], 7)
        self.assertEqual(cohort["upgrade_queue"][0]["id"], "au_finance_data_breach_midmarket")
        self.assertEqual(cohort["upgrade_queue"][0]["blocker_count"], 4)
        ...
        self.assertEqual(by_id["de_industrial_ransomware_midmarket"]["metrics"]["source_backed_parameters"], 4)
        self.assertEqual(by_id["sg_finance_bec_midmarket"]["metrics"]["source_backed_parameters"], 1)
        ...
        self.assertEqual(by_id["au_finance_data_breach_midmarket"]["metrics"]["confidence_medium_or_high_parameters"], 4)
        self.assertIn("blockers=4", output)
    ...
    def test_seeded_sprint_report_groups_actionable_upgrade_work(self):
        ...
        self.assertEqual(sprint["target_count"], 7)
        self.assertEqual(sprint["total_blocker_count"], 101)
        ...
        self.assertNotIn("sources/registry.yaml + sources/manifest.json refresh", by_id["au_finance_data_breach_midmarket"]["required_artifacts"])
        self.assertIn("python scripts/riskshard_modules.py propose au_finance_data_breach_midmarket", by_id["au_finance_data_breach_midmarket"]["review_commands"])
        self.assertIn("Seeded Evidence Upgrade Sprint A", output)
        self.assertIn("Acceptance criteria", output)
        self.assertIn("au_finance_data_breach_midmarket: blockers=4", output)
