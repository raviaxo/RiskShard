...
class BetaReadinessTests(unittest.TestCase):
    def test_beta_readiness_gate_prioritizes_product_depth_before_ops(self):
        ...
        self.assertEqual(report["gate"]["status"], "not_beta_ready")
        self.assertEqual(report["gate"]["passed"], 8)
        self.assertEqual(report["gate"]["total"], 11)
        self.assertEqual(checks["BETA-DATA-01"]["status"], "pass")
        self.assertEqual(checks["BETA-DATA-02"]["status"], "fail")
        self.assertEqual(checks["BETA-DATA-02"]["actual"], 6)
        self.assertEqual(checks["BETA-DATA-02"]["target"], 10)
        self.assertEqual(checks["BETA-DATA-03"]["status"], "fail")
        self.assertEqual(checks["BETA-DATA-03"]["actual"], 1)
        self.assertEqual(checks["BETA-DATA-03"]["target"], 3)
        self.assertEqual(checks["BETA-ENG-03"]["status"], "fail")
        self.assertEqual(checks["BETA-GOV-01"]["status"], "pass")
        self.assertEqual(checks["BETA-GOV-02"]["status"], "pass")
        self.assertIn("Seed 4 more governed starter modules", output)
        self.assertIn("Promote 2 more modules", output)
